...
def get_logger(filename='logger-speakerDependent',level='info', outPath='./'):
...
def worker_oneSpk(args, dataFolder):
    ...
    loader_train = dataloader.build_dataloader_folder(folder_train, args.batch_size, 'train', channels=args.input_channels, num_workers=0)
    folder_test  = os.path.join(dataFolder, 'test')
    loader_test  = dataloader.build_dataloader_folder(folder_test,  args.batch_size, 'test',  channels=args.input_channels, num_workers=0)
